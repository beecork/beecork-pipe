...
async function handleMediaGeneration(
  ...
  const { prompt, style, duration, provider } = (args || {}) as {
    prompt: string;
    style?: string;
    duration?: number;
    provider?: string;
  ...
  try {
    ...
    PendingMessageStore.enqueueMedia(
      'default',
      { type: 'media', filePath: result.filePath, caption: prompt.slice(0, 200) },
      ctx.db,
    );
    ...
    return fail(
      `${mediaType} generation failed: ${err instanceof Error ? err.message : String(err)}`,
    );
  }
}
...
export const HANDLERS: Record<string, Handler> = {
  beecork_remember: async (ctx, args) => {
    ...
    if (scope && scope !== 'tab' && scope !== 'auto') {
      ...
      addKnowledge(content, scope as 'global' | 'project', {
        category,
  ...
  beecork_task_create: async (ctx, args) => {
    ...
    if (!VALID_SCHEDULE_TYPES.includes(scheduleType)) {
      return fail(
        `Invalid scheduleType "${scheduleType}". Must be one of: ${VALID_SCHEDULE_TYPES.join(', ')}`,
      );
    }
  ...
  beecork_task_list: async (ctx) => {
    const jobs = ctx.db.prepare('SELECT * FROM tasks ORDER BY created_at LIMIT 500').all() as Array<
  ...
  beecork_watch_create: async (ctx, args) => {
    ...
    try {
      ...
      return ok(
        `Watcher created: "${watchName}" (${watchSchedule})\nID: ${watchId}\nWARN: signal-reload failed (${err instanceof Error ? err.message : String(err)}); restart daemon to schedule.`,
      );
    }
  ...
  beecork_tab_create: async (ctx, args) => {
    ...
    if (templateName && !template) {
      return fail(
        `Template "${templateName}" not found. Available: ${Object.keys(config.tabTemplates || {}).join(', ') || 'none'}`,
      );
    }
    ...
    try {
      ...
      if (tabSystemPrompt)
        parts.push(
          `System prompt: "${tabSystemPrompt.slice(0, 100)}${tabSystemPrompt.length > 100 ? '...' : ''}"`,
        );
  ...
  beecork_tab_list: async (ctx) => {
    ...
    const lines = tabs.map(
      (t) => `- ${t.name} [${t.status}] dir:${t.workingDir} last:${t.lastActivityAt}`,
    );
    return ok(lines.join('\n'));
  },
  ...
  beecork_recall: async (ctx, args) => {
    ...
    if (typeof query !== 'string' || query.length === 0) return fail('query is required');
    if (query.length > 256) return fail('query is too long (max 256 chars)');
  ...
  beecork_status: async (ctx) => {
    ...
    const taskCount = (
      ctx.db.prepare('SELECT COUNT(*) as c FROM tasks WHERE enabled = 1').get() as { c: number }
    ).c;
    const watcherCount = (
      ctx.db.prepare('SELECT COUNT(*) as c FROM watchers WHERE enabled = 1').get() as { c: number }
    ).c;
    const memoryCount = (
      ctx.db.prepare('SELECT COUNT(*) as c FROM memories').get() as { c: number }
    ).c;
    return ok(
      [
  ...
  beecork_send_media: async (ctx, args) => {
    const { filePath, caption, tabName } = (args || {}) as {
      filePath: string;
      caption?: string;
      tabName?: string;
    };
  ...
  beecork_failed_deliveries: async (ctx) => {
    ...
    const lines = failed.map(
      (f) =>
        `[${f.created_at}] tab:${f.tab_name} retries:${f.retry_count}\n  ${f.content.slice(0, 200)}`,
    );
    return ok(lines.join('\n\n'));
  },
  ...
  beecork_handoff: async (_ctx, args) => {
    ...
    if (!exported) return fail(`Tab "${tabName}" not found`);
    const info = {
  ...
  beecork_delegate: async (ctx, args) => {
    const { tabName, message, returnToTab } = (args || {}) as {
      tabName: string;
      message: string;
      returnToTab?: string;
    };
  ...
  beecork_delegation_status: async (_ctx, args) => {
    ...
    const lines = delegations.map(
      (d) =>
        `${d.sourceTab} → ${d.targetTab} [${d.status}] (depth ${d.depth})\n  "${d.message.slice(0, 100)}"`,
    );
    return ok(lines.join('\n\n'));
  },
  ...
  beecork_project_list: async () => {
    ...
    const lines = projects.map(
      (p) => `${p.type === 'category' ? '📁' : '📦'} ${p.name} — ${p.path}`,
    );
    return ok(lines.join('\n'));
  },
  ...
  beecork_capabilities: async () => {
    ...
    const lines = packs.map(
      (p) =>
        `${isEnabled(p.id) ? '✓ enabled' : '○ available'} | ${p.id} — ${p.name}: ${p.description}`,
    );
    return ok(lines.join('\n'));
  },

  beecork_history: async (_ctx, args) => {
    const { date, tabName, limit } = (args || {}) as {
      date?: string;
      tabName?: string;
      limit?: number;
    };
  ...
  beecork_store_search: async (_ctx, args) => {
    ...
    try {
      const response = await fetch(
        `https://registry.npmjs.org/-/v1/search?text=beecork+${encodeURIComponent(query)}&size=10`,
      ...
      const lines = packages.map(
        (o) =>
          `${o.package.name}@${o.package.version} — ${o.package.description || 'No description'}`,
      );
      return ok(
